...
namespace TestFramework
{
    public static class GraphBrowser
    {
        ...
        public static string QueryString
        {
            get
            {
                return GraphUtility.GetConfigurationValue("MSGraphQueryString");
            }
        }

        //Zooms screen to the parameter percent, then resizes window to that screen size 
        //Needed for tests which depend upon screen width > 1000px, since deployment machine width is that small 
        public static void ZoomToPercent(int percent) 
        { 
            IJavaScriptExecutor js = webDriver as IJavaScriptExecutor;
            js.ExecuteScript("document.body.style.zoom = '" + percent + "%';");
            js.ExecuteScript("$(window).resize()"); 
        } 

        public static void Initialize()
        {
            SetWaitTime(TimeSpan.FromSeconds(GraphUtility.DefaultWaitTime * 6));
