...
{
    public class CardTryItOut : GraphBasePage
    {
        ...
        {
            var responseBody = GraphBrowser.Driver.FindElement(By.Id("responseBody"));
            ...
            var wait = new WebDriverWait((IWebDriver)GraphBrowser.Driver, TimeSpan.FromSeconds(GraphUtility.DefaultWaitTime * 3));
        ...
        public bool UrlContainsServiceName()
        {
            if (IsParameterTableDisplayed())
            {
                switch (currentSerivce)
                {
                    ...
                    case ServiceToTry.GetEvents:
                    case ServiceToTry.GetContacts:
                        return GraphBrowser.Driver.FindElement(By.Id("urlValue")).Text.Contains(EnumExtension.GetDescription(currentSerivce));
                    case ServiceToTry.GetFiles:
                    case ServiceToTry.GetUsers:
                    case ServiceToTry.GetGroups:
            ...
            else
            {
                switch (currentSerivce)
                {
                    ...
                    case ServiceToTry.GetContacts:
                        return GraphBrowser.Driver.FindElement(By.Id("urlValue")).Text.Contains(EnumExtension.GetDescription(currentSerivce));
                    case ServiceToTry.GetFiles:
                        return GraphBrowser.Driver.FindElement(By.Id("urlValue")).Text.Contains("drive/root/children");
                    case ServiceToTry.GetUsers:
                        return GraphBrowser.Driver.FindElement(By.Id("urlValue")).Text.Contains("me");
                    case ServiceToTry.GetGroups:
                        return GraphBrowser.Driver.FindElement(By.Id("urlValue")).Text.Contains("me/memberOf");
                    default:
                        return false;
                }
            }
        ...
        public bool UrlContainsParameterValue()
        {
            ...
            else
            {
                ...
                {
                    case ServiceToTry.GetMessages:
                        return GraphBrowser.Driver.FindElement(By.Id("urlValue")).Text.Contains(GetMessagesValue.Inbox.ToString());
                    case ServiceToTry.GetFiles:
                        return GraphBrowser.Driver.FindElement(By.Id("urlValue")).Text.Contains("drive/root/children");
                    case ServiceToTry.GetUsers:
                        return GraphBrowser.Driver.FindElement(By.Id("urlValue")).Text.Contains("me");
        ...
        {
            IWebElement parameterDetail = GraphBrowser.Driver.FindElement(By.Id("parameterDetails"));
